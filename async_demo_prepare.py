...
async def run_async(async_client):
    # async_client = AsyncMilvusClient(uri="example.db")
    vectors_to_search = rng.random((1, dim))
    results = await async_client.async_search(collection_name, vectors_to_search, limit=3, output_fields=["id"])
    return results

results = asyncio.gather(run_async(async_client))
print(results)
