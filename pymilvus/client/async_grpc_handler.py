...
class AsyncGrpcHandler:
    ...
    def _wait_for_channel_ready(self, timeout: Union[float] = 10, retry_interval: float = 1):
        ...
        try:
            ...
            # grpc.aio.secure_channel().get_state() != grpc.ChannelConnectivity.READY
            retry_time = int(timeout / retry_interval)
            # retry_interval = 1 # 重试时间间隔（单位为秒），放到函数参数里
            while self._async_channel.get_state(try_to_connect=True) != grpc.ChannelConnectivity.READY:
                time.sleep(retry_interval) # 等待一秒后重试
                retry_time -= 1
                if retry_time <= 0:
                    raise MilvusException(
                        code=Status.CONNECT_FAILED,
                        message=f"Fail connecting to server on {self._address}, illegal connection params or server unavailable, reach maximum retry time",
                    )
